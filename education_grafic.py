...
class Sections:
        ...
        while not self.game_finished and not self.game_again:
            ...
            if self.click_flag and time.time() - self.click_time > 0.5:
                if self.figures_active:
                    ...
                    if 43 < chance <= 76:
                        bot_figure = self.human_figure
                    elif self.human_figure == 1:
                        if chance <= 43:
                            bot_figure = 2
                        elif chance > 76:
                            bot_figure = 3
                    elif self.human_figure == 2:
                        if chance <= 43:
                            bot_figure = 3
                        elif chance > 76:
                            bot_figure = 1
                    elif self.human_figure == 3:
                        if chance <= 43:
                            bot_figure = 1
                        elif chance > 76:
                            bot_figure = 2
    ...
    def profile(self):
        pygame.display.update()
        clock = pygame.time.Clock()
        self.profile_finished, self.login, self.profile_login, enter = False, False, True, False
